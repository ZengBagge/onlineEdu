...
function del(id,dom){
if(confirm("确定要删除么？")){
 addTopicFill.del(id,function(result){
   if(result){
...
function update(id){
  $("#addTopic_iframe").attr("src","topicFill!update?fillTopicId="+id);
}
</script>
  </head>
  ...
  <body>
      ...
      <s:iterator value="pageBean.list" var="u">
          <tr>
            ...
            <td>
              <s:iterator value="#u.answers" var="a2">
                 (<s:property  value="#a2.orderInt" />) <s:property  value="#a2.body" /> <br/>
              </s:iterator>
            </td>
...
　　　    <li ><a href="topicFill!getFillList?courseId=<s:property value="%{courseId}"/>&page=<s:property value="%{pageBean.currentPage+1}"/>">下一页<span aria-hidden="true">&rarr;</span></a></li>
      </s:if>
      <s:elseif test="%{pageBean.currentPage < pageBean.totalPage}">
    <li ><a href="topicFill!getFillList?courseId=<s:property value="%{courseId}"/>&page=<s:property value="%{pageBean.currentPage-1}"/>"><span aria-hidden="true">&larr;</span>上一页</a></li>
    <li ><a href="topicFill!getFillList?courseId=<s:property value="%{courseId}"/>&page=<s:property value="%{pageBean.currentPage+1}"/>">下一页<span aria-hidden="true">&rarr;</span></a></li>
       </s:elseif>
       　<s:else>
　　　　    <li ><a href="topicFill!getFillList?courseId=<s:property value="%{courseId}"/>&page=<s:property value="%{pageBean.currentPage-1}"/>"><span aria-hidden="true">&larr;</span>上一页</a></li>
