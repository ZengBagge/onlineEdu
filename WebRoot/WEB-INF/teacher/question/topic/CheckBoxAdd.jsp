     ...
     <body class="fram_body">
           <div class="course_main">
               <form action="course!addForm" method="post" class=" form-horizontal">
                            ...
                            <div class="col-sm-8">
                           <input type="text" class="form-control" id="topicTid" placeholder="编号，格式（××-××-××××）" check-type="required" required-message="请填写编号" value="<s:property value="checkBoxTopic.topic.tid" />"/>
                            </div>
                    </div>
                    <div class="form-group">
                            <label for="topicTitle" class="col-sm-2 control-label">题干</label>
                            <div class="col-sm-8">
                           <input type="text" class="form-control" id="topicTitle" placeholder="题干" check-type="required" required-message="请填写题干" value="<s:property value="checkBoxTopic.topic.titleString" />"/>
                                                     <s:if test="checkBoxTopic !=null" ><input type="hidden" id="id" value="<s:property value="checkBoxTopic.id" />"></s:if>
                    ...
                    <div class="form-group">
                           ...
                           <select class="form-control" id="courseChapter"  >      
                            <s:action name="courseChapter!getChapterList" namespace="/" var="t"  >
                             ...
                             <option value="<s:property  value="checkBoxTopic.topic.chapter.id" />"><s:property  value="checkBoxTopic.topic.chapter.title" /></option>
                     ...
                     <div class="form-group">
                           ...
                           <select class="form-control" id="topicType"  >      
                            <s:action name="topicType!getTopicType" namespace="/" var="t"  ></s:action>
                             <option value="<s:property  value="checkBoxTopic.topic.topicType.id" />"><s:property  value="checkBoxTopic.topic.topicType.name" /></option>
                      ...
                      <s:iterator value="checkBoxTopic.answers" var="a2">
                     <div class="form-group">
                            <label for="topicAnswerA" class="col-sm-2 control-label">选项</label>
                            <div class="col-sm-8">
                           <input type="text" class="form-control" id="topicAnswer<s:property value="#a2.orderInt" />" placeholder="选项" check-type="required" required-message="请填写选项" value="<s:property value="#a2.body" />" />
                            <label for="as<s:property value="#a2.orderInt" />" class="control-label">是否为答案</label><input type="checkbox"  data-id="<s:property value="#a2.orderInt" />" class="checkbox"/>
                            </div>
                    </div>
                    </s:iterator>
                    ...
                    <div class="form-group">
                            ...
                            <div class="col-sm-8">
                            <textarea id="topicDescription" name="topicDescription" class="form-control" placeholder="题目解析 "><s:property value="checkBoxTopic.topic.description" /></textarea>
                            </div>
                    </div >
                    <div class="form-group">
                            <label for="topicDescription" class="col-sm-2 control-label">题目来源</label>
                            <div class="col-sm-8">
                            <textarea id="topicSource" name="topicSource" class="form-control" placeholder="题目来源 "><s:property value="checkBoxTopic.topic.sourceString" /></textarea>
                            </div>
                    </div >
                    <div class="form-group">
                      <label for="courseIntroduce" class="col-sm-2 control-label"></label>
                            <div class="col-sm-8">
                    <s:if test="checkBoxTopic !=null"><button type="button" id="course_btn_update" class="btn btn-info btn-course" >更新</button></s:if><s:else>       
			         ...
			         var answerA=$("#topicAnswerA").val();
			            var answerB=$("#topicAnswerB").val();
			               var answerC=$("#topicAnswerC").val();
			                  ...
			                  var answerTrue=new Array();
			                 $(".checkbox").each(function(){
			                     if ($(this).is(":checked"))
			                        {
			                          answerTrue.push($(this).attr("data-id"));  
			                        }
			                 });
			          ...
			          addTopicCheckBox.addForm(title,answerTrue,answers,decription,topicType,topicSource,courseChapter,tid,courseId,function(result){
                         if(result){
...
<script type="text/javascript">
			 $(function(){
			   ...
			   $("#course_btn_update").on('click',function(event){
			    ...
			    {  
			         var title=$("#topicTitle").val();
			         var answerA=$("#topicAnswer0").val();
			            var answerB=$("#topicAnswer1").val();
			               var answerC=$("#topicAnswer2").val();
			                  var answerD=$("#topicAnswer3").val();
			                  var answers = [answerA,answerB,answerC,answerD];
			                  var answerTrue=new Array();
			                 $(".checkbox").each(function(){
			                     if ($(this).is(":checked"))
			                  ...
			                  var topicSource =$("#topicSource").val();
			                  var courseChapter =$("#courseChapter").val();
			          addTopicCheckBox.updateFrom(id,title,answerTrue,answers,decription,topicType,topicSource,courseChapter,tid,function(result){
                         if(result){
